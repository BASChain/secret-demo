...
ctx.fillText( 'Big Hugi', w / 2 - ctx.measureText( 'Big Hugi' ).width / 2, h / 2 - 15 );
...
function timeline(){
  var threshold_position = window.scrollY + window.innerHeight * 1/5;
  //compare scrolltop with scrolltop on each timeline event
  var timeline_events = $('.about-timeline li');
  $('.about-timeline li').each((index,el) => {
    if(el.offsetTop < threshold_position){
      console.log(el);
      $(el).addClass('visible');
    }else{
